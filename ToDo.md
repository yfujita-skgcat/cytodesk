...
- [x] current-view exportがQtのactive/overlay実効Z値を保持し、Samplesリスト順のGUIドット重なりを`source_draw_order`へ反映するよう修正した。semantic title orderは変更せず、GUIと右クリックexportの前面サンプルを一致させる。
- [x] Batch Plot Exportのmanual overlayも、active sampleを含む全sourceをSamplesリストのQt Z順（下側から上側）で描画するようcore helperへ統一した。タイトル/metadataのsemantic orderは維持し、3-source回帰testでGUIと同じ前面sourceを検証する。
- [x] advanced overlayのexport layerにも永続`source_id`を保持し、Qt上では表示されるがcurrent-view exportのsource-ID mapから欠落する経路を防いだ。実ウィジェットsnapshotとPNGの重なり中心色を比較する回帰testを追加した。
...
- [x] Batch overlayのタイトル順をSamples listのsemantic orderへ統一する。manual overlayはタイトル・metadataを元の順序で保持し、ドットの前後関係は全source（activeを含む）を`source_draw_order`でSamples listの下側から上側へ解決する。3-source回帰テストでGUI相当のタイトル順と描画順を別々に検証する。
